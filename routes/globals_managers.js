...
router.get('/', function(req, res, next) {
    req.getConnection(function(error, conn) {
        conn.query('SELECT * FROM globals_managers ORDER BY id_global_manager DESC',function(err, rows, fields) {
            //if(err) throw err
            if (err) {
                req.flash('error', err)
                res.render('global_manager/list', {
                    title: 'Global managers list', 
                    data: ''
                })
            } else {
                res.render('global_manager/list', {
